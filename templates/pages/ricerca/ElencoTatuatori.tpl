{extends file='layouts/base_ricerca.tpl'}

{block name="title"}Risultati ricerca — InkMaster{/block}
    ...
    <div class="el-list">
      ...
      {else}
        {foreach $data as $studio}
          {assign var=nomeStudio value=$studio->getNome()}
          <a href="/scegli_studio?id={$studio->getId()}" class="el-card">

            <div class="el-avatar">{$nomeStudio|substr:0:2|upper}</div>

            <div class="el-info">
              <div class="el-nome">{$nomeStudio|escape}</div>
              {if $studio->getDescrizione()}
                <div class="el-desc">{$studio->getDescrizione()|escape}</div>
              {/if}
              <div class="el-tags">
                {foreach $studio->getTatuatori() as $tat}
                  {foreach $tat->getStili() as $st}
                    <span class="el-tag">{$st->getNome()|escape}</span>
                  {/foreach}
                {/foreach}
              </div>
              <div class="el-tat">
                {foreach $studio->getTatuatori() as $tat}
                  {$tat->getNome()|escape} {$tat->getCognome()|escape}{if !$tat@last}, {/if}
                {/foreach}
              </div>
            </div>
            ...
            <div class="el-meta">
              ...
              <div class="el-location">
                ...
                <span class="el-city">{$studio->getPosizione()->value|escape}</span>
              </div>
